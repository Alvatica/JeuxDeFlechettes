...
public class FlechetteGUI extends JPanel{
	...
	*/
			...
			public void mouseClicked(MouseEvent e) {
		        ...
		        for (int i=0; i<shapes.length-20; i++) {//Ommit the last 20 slices as they contain the exterior black slices (numbers) of the target
		        	...
		        	if (s.shape.contains(x, y)) {
		        		...
		        		/*final ColoredShapeEntry clickedShape = s;
		        		...
		        		blinkTimer.schedule(new TimerTask() {
		        			...
		        			public void run() {
		        				if (count==0) {
		        					originalColor = clickedShape.color;
		        				}
		        				Color paintColor = Color.ORANGE;
		        				if (count%2==1) {
	...
	public void afficherFlechette(Flechette flechette) {
		...
		if (shape==null)
			return;
		blinkSector(shape, Color.ORANGE, 5,200);
	}
	
	public void afficherRecommandationPourJoueur(Joueur joueur, Flechette flechetteRecommander) {
		if (flechetteRecommander==null)
			return;
		ColoredShapeEntry shape = getSector(flechetteRecommander);
		if (shape==null)
			return;
		blinkSector(shape, Color.YELLOW/*decode("#79FA38")*/, 11,300);
	}
	private void blinkSector(ColoredShapeEntry shape, Color color, int countBlincks, long period) {
		final ColoredShapeEntry clickedShape = shape;
		...
		blinkTimer.schedule(new TimerTask() {
			...
			public void run() {
				if (count==0) {
					originalColor = clickedShape.originalColor;
				}
				Color paintColor = animColor;
				if (count%2==1) {
					paintColor=originalColor;
				}				
				clickedShape.color=paintColor;
